...
const itemTentatives = document.querySelector('.numberTentatives');
let compteur = 0;
let step = 1;
let nbTentatives = 12;
...
const drop = (event) => {
    ...
    if(compteur % 4 == 0) {
        ...
        itemTentatives.innerText = nbTentatives;
...
const descoverResult = () => {
    circleChoices.forEach((el) => {
        el.classList.remove('hide');

        setTimeout(() => {
            modal.classList.add('modalIn');
            modal.children[0].innerText = "Perdu ! Vous avez cliquer sur le bouon découvrir la combinaison secrète.";
            content.classList.add("content__modal");
        }, 1000);
    });
};